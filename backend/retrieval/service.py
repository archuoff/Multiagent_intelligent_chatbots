...
from backend.retrieval.query_embedding import embed_query
...
class RetrievalService:
    ...
    def __init__(self, vector_store: QdrantVectorStore, embedding_provider: AzureOpenAIEmbeddingProvider,
                 canonical_store: CanonicalArtifactStore) -> None:
    ...
    def retrieve(self, *, agent_id: str, query_text: str, principal_group_codes: frozenset[str],
        ...
        """Embeds the query, searches one agent's collection, and drops any chunk this principal can't see."""
        query_vector = embed_query(query_text, provider=self._embedding_provider)
        raw_results = self._vector_store.search(agent_id=agent_id, query_vector=query_vector, limit=limit)
        chunks = [
